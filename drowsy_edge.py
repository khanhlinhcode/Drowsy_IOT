...
MQTT_STATUS_QOS = 1
MQTT_META_QOS = 1
MQTT_MIN_PUB_GAP_MS = 120     # 8.3Hz max publish rate
MQTT_RETRY_MIN_S = 1
MQTT_RETRY_MAX_S = 8
MQTT_ERR_LOG_GAP_MS = 3000
MQTT_STATE_TTL_MS = 1400
MQTT_LOG_GAP_MS = 1500
MQTT_META_MIN_MS = 120
...
def _init_mqtt():
    ...
    mqtt_client.max_queued_messages_set(120)
...
def _publish_signal(status, signal, fatigue, now_ms):
    ...
    forced_sync = _mqtt_force_sync
    status_changed = (status != _last_sent) or (sig != _last_sent_signal)
    elapsed_ms = now_ms - _last_sent_ms
    danger_escalation = (
        (sig > _last_sent_signal and sig >= 2)
        or (status == "MICROSLEEP" and _last_sent != "MICROSLEEP")
    )

    # Real-time mode: publish only on state/signal changes (or forced sync after reconnect).
    if not forced_sync and not status_changed:
        return

    # Debounce repeated transitions to avoid flooding while keeping danger escalation immediate.
    if status_changed and (not danger_escalation) and (not forced_sync) and elapsed_ms < MQTT_MIN_PUB_GAP_MS:
        return
    ...
    if status_changed or forced_sync or status != _last_mqtt_log_status or (now_ms - _last_mqtt_log_ms) >= MQTT_LOG_GAP_MS:
